 ...
 * @version     $Id: shSec.php 2050 2011-06-30 13:52:38Z silianacom-svn $
 *
 */
...
function shDoRestrictedAccess( $causeText, $comment = '', $displayEntrance = false) {
  ...
  if ($sefConfig->shSecLogAttacks) {  // log what's happening
    $sep = "\t";
    $causeText = shSecOutput( $causeText);
    ...
    $logData .= $userAgent.$sep.$_SERVER['REQUEST_METHOD'].$sep.$_SERVER['REQUEST_URI'].$sep.$comment;
    $logData .="\n";
...
function shCheckScripts( $query) {
  if (empty($query)) return '';
  if (preg_match( '/(\<).*script[^A-Za-z0-9]*(\>)/i', $query))
  return '<script> tag';
  else return;
}
