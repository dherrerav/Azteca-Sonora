...
****************************************************************************
ds-syndicate 1.1.0
	Use 'r' date() format for rfc822 dates (uses server time zone, PHP 4.0.4 or later)
	define TIME_ZONE only if not already defined (not used for rfc822 dates).
	class FeedCreator: added instance variable $cssStyleSheet = ""; already referenced in one of its functions.
	saveFeed(): if write error, also logs to web server error log
	When appropriate, return HTTP 304 Not Modified instead of redirected file.
	Log that oh-so-common permissions error
	RSSCreator20(): set contentType.
...
class FeedCreator extends HtmlDescribable {
	...
	*/
	...
	// needed for _createStylesheetReferences()
	var $cssStyleSheet = "";
	...
	function useCached($filename="", $timeout=3600) {
		...
		if (file_exists($filename) AND (time()-filemtime($filename) < $timeout)) {
			// But first: Conditional GET: Use HTTP 1.1 field to see if client already has latest version
  			doConditionalGet(filemtime($filename));	// May exit
	...
	function saveFeed($filename="", $displayContents=true) {
		...
		if ($feedFile) {
			...
			error_log("FeedCreator: Error creating feed file, please check write permissions.",0);
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "        <item>\n";
			$feed.= "            <title>".FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)),100)." (".FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->voted)),10)  .") </title>\n";
			$feed.= "            <link>".htmlspecialchars($this->items[$i]->link)."</link>\n";
...
// Given $timestamp is modify date of the requested xml file, if client already has it, return HTTP 304 and quit
