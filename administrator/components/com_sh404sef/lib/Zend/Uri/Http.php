 ...
 * @version   $Id: Http.php 1757 2011-01-30 15:53:15Z silianacom-svn $
 */
...
/**
 * @see Sh_Zend_Validate_Hostname
 */
/*
 * Shumisha : simplify
 * require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Validate/Hostname.php';*/
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class Sh_Zend_Uri_Http extends Sh_Zend_Uri
{
    /**
     ...
     * @throws Sh_Zend_Uri_Exception When the URI is not valid
     */
    protected function __construct($scheme, $schemeSpecific = '')
    {
        ...
        // Validate the URI
        if ($this->valid() === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Invalid URI supplied');
        }
    }

    /**
     * Creates a Sh_Zend_Uri_Http from the given string
     *
     ...
     * @throws Sh_Zend_Uri_Exception        When the given $uri is invalid
     * @return Sh_Zend_Uri_Http
     */
    public static function fromString($uri)
    {
        if (is_string($uri) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('$uri is not a string');
        }
        ...
        if (in_array($scheme, array('http', 'https')) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Invalid scheme: '$scheme'");
        }

        $schemeHandler = new Sh_Zend_Uri_Http($scheme, $schemeSpecific);
     ...
     * @throws Sh_Zend_Uri_Exception When scheme-specific decoposition fails
     * @throws Sh_Zend_Uri_Exception When authority decomposition fails
     * @return void
     */
    protected function _parseUri($schemeSpecific)
    {
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: scheme-specific decomposition failed');
        }
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: authority decomposition failed');
        }
     ...
     *
     * @throws Sh_Zend_Uri_Exception When one or more parts of the URI are invalid
     * @return string
     */
    public function getUri()
    {
        if ($this->valid() === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('One or more parts of the URI are invalid');
        }
     ...
     * @throws Sh_Zend_Uri_Exception When username validation fails
     * @return boolean
    ...
    public function validateUsername($username = null)
    {
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: username validation failed');
        }
     ...
     *
     * @param  string $username The HTTP username
     * @throws Sh_Zend_Uri_Exception When $username is not a valid HTTP username
     * @return string
     */
    public function setUsername($username)
    {
        if ($this->validateUsername($username) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Username \"$username\" is not a valid HTTP username");
        }
     ...
     * @throws Sh_Zend_Uri_Exception When password validation fails
     * @return boolean
     * @link   http://www.faqs.org/rfcs/rfc2396.html
     */
    public function validatePassword($password = null)
    {
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: password validation failed.');
        }
     ...
     *
     * @param  string $password The HTTP password
     * @throws Sh_Zend_Uri_Exception When $password is not a valid HTTP password
     * @return string
     */
    public function setPassword($password)
    {
        if ($this->validatePassword($password) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Password \"$password\" is not a valid HTTP password.");
        }
    ...
    public function validateHost($host = null)
    {
        ...
        /*
         * Shumisha : simplify
         * // Check the host against the allowed values; delegated to Sh_Zend_Filter.
        $validate = new Sh_Zend_Validate_Hostname(Sh_Zend_Validate_Hostname::ALLOW_ALL);
     ...
     *
     * @param  string $host The HTTP host
     * @throws Sh_Zend_Uri_Exception When $host is nota valid HTTP host
     * @return string
     */
    public function setHost($host)
    {
        if ($this->validateHost($host) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Host \"$host\" is not a valid HTTP host");
        }
     ...
     *
     * @param  string $port The HTTP port
     * @throws Sh_Zend_Uri_Exception When $port is not a valid HTTP port
     * @return string
     */
    public function setPort($port)
    {
        if ($this->validatePort($port) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Port \"$port\" is not a valid HTTP port.");
        }
     ...
     * @throws Sh_Zend_Uri_Exception When path validation fails
     * @return boolean
     */
    public function validatePath($path = null)
    {
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: path validation failed');
        }
     ...
     *
     * @param  string $path The HTTP path
     * @throws Sh_Zend_Uri_Exception When $path is not a valid HTTP path
     * @return string
     */
    public function setPath($path)
    {
        if ($this->validatePath($path) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Path \"$path\" is not a valid HTTP path");
        }
     ...
     * @throws Sh_Zend_Uri_Exception When query validation fails
     * @return boolean
     * @link   http://www.faqs.org/rfcs/rfc2396.html
     */
    public function validateQuery($query = null)
    {
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: query validation failed');
        }
     ...
     * @throws Sh_Zend_Uri_Exception When $query is not a valid query string
     * @return string              Old query string
     */
    public function setQuery($query)
    {
        ...
        if ($this->validateQuery($query) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("'$query' is not a valid query string");
        }
     ...
     * @throws Sh_Zend_Uri_Exception When fragment validation fails
     * @return boolean
     * @link   http://www.faqs.org/rfcs/rfc2396.html
     */
    public function validateFragment($fragment = null)
    {
        ...
        if ($status === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception('Internal error: fragment validation failed');
        }
     ...
     * @throws Sh_Zend_Uri_Exception When $fragment is not a valid HTTP fragment
     * @return string
     */
    public function setFragment($fragment)
    {
        if ($this->validateFragment($fragment) === false) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri/Exception.php';
            throw new Sh_Zend_Uri_Exception("Fragment \"$fragment\" is not a valid HTTP fragment");
        }
