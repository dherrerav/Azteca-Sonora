...
function jasdl_initJAScriptDLMenu() {
	...
	//Set active item
	for (i=0;i<jasdl_activemenu.length;i++) {
		actitem = document.getElementById("jasdl-mainnav"+jasdl_activemenu[i].toString())
		if (actitem)
		{
			if (actitem.className) actitem.className += " active"; else actitem.className = "active";
		}
		actitem = document.getElementById("jasdl-subnavitem"+jasdl_activemenu[i].toString())
		if (actitem)
		{
			if (actitem.className) actitem.className += " active"; else actitem.className = "active";
		}	
		subx = document.getElementById("jasdl-subnav"+jasdl_activemenu[i].toString())
		if (subx) {
			subx.style.display = "block";
			jasdl_recover = jasdl_activemenu[i]
		}
	}
