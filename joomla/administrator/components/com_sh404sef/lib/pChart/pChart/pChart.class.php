...
// MHB : modification spécifique
define("PIE_PERCENTAGE_LABEL_VALUE", 999);
  ...
  function clearScale()
  {
    ...
    function drawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
    {
      ...
      if ( $this->VMin == NULL && $this->VMax == NULL)
      {
        ...
        if ( $ScaleMode == SCALE_NORMAL || $ScaleMode == SCALE_START0 )
        {
          ...
          // shumisha adjust scaling: if SCALE_NORMAL, don't start at actual MIN value
          // but a few percent below, so that smallest value is not exactly on the abscise axis
          if( $this->VMin > 0) {
            $this->VMin = $this->VMin - (0.02 * ($this->VMax - $this->VMin));
            $this->VMin = $this->VMin < 0 ? 0 : $this->VMin;
          }
    ...
    function drawRadarAxis($Data,$DataDescription,$Mosaic=TRUE,$BorderOffset=10,$A_R=60,$A_G=60,$A_B=60,$S_R=200,$S_G=200,$S_B=200,$MaxValue=-1)
    {
      ...
      if ( $Mosaic )
      {
        ...
        for ( $t=1; $t<=$MaxValue-1; $t++)
        {
          ...
          for ( $i=0; $i<=$Points; $i++)
          {
            ...
            $X1 = cos($Angle * 3.14159265358979 / 180 ) * $TRadius + $XCenter;
            $Y1 = sin($Angle * 3.14159265358979 / 180 ) * $TRadius + $YCenter;
            $X2 = cos($Angle * 3.14159265358979 / 180 ) * ($TRadius+$RadiusScale) + $XCenter;
            $Y2 = sin($Angle * 3.14159265358979 / 180 ) * ($TRadius+$RadiusScale) + $YCenter;
      ...
      for ( $t=1; $t<=$MaxValue; $t++)
      {
        ...
        {
          $Angle = -90 + $i * 360/$Points;
          $X = cos($Angle * 3.14159265358979 / 180 ) * $TRadius + $XCenter;
          $Y = sin($Angle * 3.14159265358979 / 180 ) * $TRadius + $YCenter;
      ...
      {
        $Angle = -90 + $i * 360/$Points;
        $X = cos($Angle * 3.14159265358979 / 180 ) * $Radius + $XCenter;
        $Y = sin($Angle * 3.14159265358979 / 180 ) * $Radius + $YCenter;
      ...
      for ( $t=1; $t<=$MaxValue; $t++)
      {
        ...
        $X1 = $XCenter;
        $Y1 = $YCenter - $TRadius;
        $X2 = cos($Angle * 3.14159265358979 / 180 ) * $TRadius + $XCenter;
        $Y2 = sin($Angle * 3.14159265358979 / 180 ) * $TRadius + $YCenter;
      ...
      foreach ( $DataDescription["Values"] as $Key2 => $ColName )
      {
        ...
        foreach ( $Data as $Key => $Values )
        {
          if ( isset($Data[$Key][$ColName]))
          {
            ...
            $XPos = cos($Angle * 3.14159265358979 / 180 ) * $Strength + $XCenter;
            $YPos = sin($Angle * 3.14159265358979 / 180 ) * $Strength + $YCenter;
    ...
    function drawBasicPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$R=255,$G=255,$B=255,$Decimals=0)
    {
      ...
      foreach($iValues as $Key => $Value)
      {
        ...
        if ( !($DrawLabels == PIE_NOLABEL) )
        {
          ...
          // MHB modification spécifique
          elseif ($DrawLabels == PIE_PERCENTAGE_LABEL_VALUE)
          $Caption  = $iLabels[$Key]."\r\n".(round($Value * pow(10,$Decimals) * $SplicePercent)/pow(10,$Decimals))."%" . ' (' . $Value . ')';

          $Position   = imageftbbox($this->FontSize,0,$this->FontName,$Caption);
          $TextWidth  = $Position[2]-$Position[0];
          $TextHeight = abs($Position[1])+abs($Position[3]);

          $TX = cos(($TAngle) * 3.14159265358979 / 180 ) * ($Radius+10) + $XPos;

          if ( $TAngle > 0 && $TAngle < 180 )
          $TY = sin(($TAngle) * 3.14159265358979 / 180 ) * ($Radius+10) + $YPos + 4;
          else
          $TY = sin(($TAngle) * 3.14159265358979 / 180 ) * ($Radius+4) + $YPos - ($TextHeight/2);
        ...
        /* Process pie slices */
        for($iAngle=$Angle;$iAngle<=$Angle+$Value*$SpliceRatio && $iAngle <= 360;$iAngle=$iAngle+.25)
        {
         
          $TopX = cos($iAngle * 3.14159265358979 / 180 ) * $Radius + $XPos;
          $TopY = sin($iAngle * 3.14159265358979 / 180 ) * $Radius + $YPos;
    ...
    function drawFlatPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$SpliceDistance=0,$Decimals=0,$AllBlack=FALSE)
    {
      ...
      foreach($iValues as $Key => $Value)
      {
        $XOffset = cos(($Angle+($Value/2*$SpliceRatio)) * 3.14159265358979 / 180 ) * $SpliceDistance;
        $YOffset = sin(($Angle+($Value/2*$SpliceRatio)) * 3.14159265358979 / 180 ) * $SpliceDistance;
        ...
        if ( !($DrawLabels == PIE_NOLABEL) )
        {
          ...
          $TX = cos(($TAngle) * 3.14159265358979 / 180 ) * ($Radius+10+$SpliceDistance) + $XPos;

          if ( $TAngle > 0 && $TAngle < 180 )
          $TY = sin(($TAngle) * 3.14159265358979 / 180 ) * ($Radius+10+$SpliceDistance) + $YPos + 4;
          else
          $TY = sin(($TAngle) * 3.14159265358979 / 180 ) * ($Radius+$SpliceDistance+4) + $YPos - ($TextHeight/2);
        ...
        for($iAngle=$Angle;$iAngle<=$Angle+$Value*$SpliceRatio && $iAngle <= 360;$iAngle=$iAngle+.25)
        {
          $PosX = cos($iAngle * 3.14159265358979 / 180 ) * $Radius + $XPos + $XOffset;
          $PosY = sin($iAngle * 3.14159265358979 / 180 ) * $Radius + $YPos + $YOffset;
    ...
    function drawPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$EnhanceColors=TRUE,$Skew=60,$SpliceHeight=20,$SpliceDistance=0,$Decimals=0)
    {
      ...
      foreach($iValues as $Key => $Value)
      {
        $XCenterPos = cos(($Angle-$CDev+($Value*$SpliceRatio+$SpliceDistanceRatio)/2) * 3.14159265358979 / 180 ) * $SpliceDistance + $XPos;
        $YCenterPos = sin(($Angle-$CDev+($Value*$SpliceRatio+$SpliceDistanceRatio)/2) * 3.14159265358979 / 180 ) * $SpliceDistance + $YPos;
        $XCenterPos2 = cos(($Angle+$CDev+($Value*$SpliceRatio+$SpliceDistanceRatio)/2) * 3.14159265358979 / 180 ) * $SpliceDistance + $XPos;
        $YCenterPos2 = sin(($Angle+$CDev+($Value*$SpliceRatio+$SpliceDistanceRatio)/2) * 3.14159265358979 / 180 ) * $SpliceDistance + $YPos;
        ...
        if ( !($DrawLabels == PIE_NOLABEL) )
        {
          ...
          if ( $TAngle > 0 && $TAngle < 180 )
          $TY = sin(($TAngle) * 3.14159265358979 / 180 ) * ($SkewHeight + 10) + $YPos + $SpliceHeight + 4;
          else
          $TY = sin(($TAngle) * 3.14159265358979 / 180 ) * ($SkewHeight + 4) + $YPos - ($TextHeight/2);
        ...
        {
          $TopX = cos($iAngle * 3.14159265358979 / 180 ) * $Radius + $XPos;
          $TopY = sin($iAngle * 3.14159265358979 / 180 ) * $SkewHeight + $YPos;
    ...
    function drawRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B)
    {
      ...
      $Step = 90 / ((3.14159265358979 * $Radius)/2);

      for($i=0;$i<=90;$i=$i+$Step)
      {
        $X = cos(($i+180)*3.14159265358979/180) * $Radius + $X1 + $Radius;
        $Y = sin(($i+180)*3.14159265358979/180) * $Radius + $Y1 + $Radius;
        $this->drawAntialiasPixel($X,$Y,$R,$G,$B);

        $X = cos(($i-90)*3.14159265358979/180) * $Radius + $X2 - $Radius;
        $Y = sin(($i-90)*3.14159265358979/180) * $Radius + $Y1 + $Radius;
        $this->drawAntialiasPixel($X,$Y,$R,$G,$B);

        $X = cos(($i)*3.14159265358979/180) * $Radius + $X2 - $Radius;
        $Y = sin(($i)*3.14159265358979/180) * $Radius + $Y2 - $Radius;
        $this->drawAntialiasPixel($X,$Y,$R,$G,$B);

        $X = cos(($i+90)*3.14159265358979/180) * $Radius + $X1 + $Radius;
        $Y = sin(($i+90)*3.14159265358979/180) * $Radius + $Y2 - $Radius;
        $this->drawAntialiasPixel($X,$Y,$R,$G,$B);
      }
    ...
    function drawFilledRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B)
    {
      ...
      for($i=0;$i<=90;$i=$i+$Step)
      {
        $Xi1 = cos(($i+180)*3.14159265358979/180) * $Radius + $X1 + $Radius;
        $Yi1 = sin(($i+180)*3.14159265358979/180) * $Radius + $Y1 + $Radius;

        $Xi2 = cos(($i-90)*3.14159265358979/180) * $Radius + $X2 - $Radius;
        $Yi2 = sin(($i-90)*3.14159265358979/180) * $Radius + $Y1 + $Radius;

        $Xi3 = cos(($i)*3.14159265358979/180) * $Radius + $X2 - $Radius;
        $Yi3 = sin(($i)*3.14159265358979/180) * $Radius + $Y2 - $Radius;

        $Xi4 = cos(($i+90)*3.14159265358979/180) * $Radius + $X1 + $Radius;
        $Yi4 = sin(($i+90)*3.14159265358979/180) * $Radius + $Y2 - $Radius;
    ...
    function drawCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0)
    {
      ...
      $Step     = 360 / (2 * 3.14159265358979 * max($Width,$Height));

      for($i=0;$i<=360;$i=$i+$Step)
      {
        $X = cos($i*3.14159265358979/180) * $Height + $Xc;
        $Y = sin($i*3.14159265358979/180) * $Width + $Yc;
        $this->drawAntialiasPixel($X,$Y,$R,$G,$B);
      }
    }
    ...
    function drawFilledCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0)
    {
      ...
      {
        $X1 = cos($i*3.14159265358979/180) * $Height + $Xc;
        $Y1 = sin($i*3.14159265358979/180) * $Width + $Yc;
        $X2 = cos((180-$i)*3.14159265358979/180) * $Height + $Xc;
        $Y2 = sin((180-$i)*3.14159265358979/180) * $Width + $Yc;
