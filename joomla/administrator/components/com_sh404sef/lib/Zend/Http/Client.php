 ...
 * @subpackage Client
 * @version    $Id: Client.php 1757 2011-01-30 15:53:15Z silianacom-svn $
 ...
 * @see Sh_Zend_Uri
 */
require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Uri.php';


/**
 * @see Sh_Zend_Http_Client_Adapter_Interface
 */
require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Adapter/Interface.php';


/**
 * @see Sh_Zend_Http_Response
 */
require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Response.php';

/**
 * @see Sh_Zend_Http_Response_Stream
 */
require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Response/Stream.php';

/**
 * Sh_Zend_Http_Client is an implemetation of an HTTP client in PHP. The client
 * supports basic features like sending different HTTP requests and handling
 * redirections, as well as more advanced features like proxy settings, HTTP
 * authentication and cookie persistance (using a Sh_Zend_Http_CookieJar object)
 *
...
class Sh_Zend_Http_Client
{
    ...
    protected $config = array(
        ...
        'useragent'       => 'Sh_Zend_Http_Client',
        'timeout'         => 10,
        'adapter'         => 'Sh_Zend_Http_Client_Adapter_Socket',
     ...
     *
     * @var Sh_Zend_Http_Client_Adapter_Interface
     */
     ...
     *
     * @param Sh_Zend_Uri_Http|string $uri
     ...
     *
     * @param  Sh_Zend_Uri_Http|string $uri
     * @return Sh_Zend_Http_Client
     * @throws Sh_Zend_Http_Client_Exception
     */
    public function setUri($uri)
    {
        if (is_string($uri)) {
            $uri = Sh_Zend_Uri::factory($uri);
        }

        if (!$uri instanceof Sh_Zend_Uri_Http) {
            /** @see Sh_Zend_Http_Client_Exception */
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
            throw new Sh_Zend_Http_Client_Exception('Passed parameter is not a valid HTTP URI.');
        }
     ...
     * @return Sh_Zend_Uri_Http|string
     */
    public function getUri($as_string = false)
    {
        if ($as_string && $this->uri instanceof Sh_Zend_Uri_Http) {
     ...
     *
     * @param  Sh_Zend_Config | array $config
     * @return Sh_Zend_Http_Client
     * @throws Sh_Zend_Http_Client_Exception
     */
    public function setConfig($config = array())
    {
        if ($config instanceof Sh_Zend_Config) {
            $config = $config->toArray();

        } elseif (! is_array($config)) {
            /** @see Sh_Zend_Http_Client_Exception */
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
            throw new Sh_Zend_Http_Client_Exception('Array or Sh_Zend_Config object expected, got ' . gettype($config));
        }
        ...
        if ($this->adapter instanceof Sh_Zend_Http_Client_Adapter_Interface) {
    ...
    public function setMethod($method = self::GET)
    {
        if (! preg_match('/^[^\x00-\x1f\x7f-\xff\(\)<>@,;:\\\\"\/\[\]\?={}\s]+$/', $method)) {
            /** @see Sh_Zend_Http_Client_Exception */
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
            throw new Sh_Zend_Http_Client_Exception("'{$method}' is not a valid HTTP request method.");
        }
    ...
    public function setHeaders($name, $value = null)
    {
        ...
        if (is_array($name)) {
            ...
            if ($this->config['strict'] && (! preg_match('/^[a-zA-Z0-9-]+$/', $name))) {
                /** @see Sh_Zend_Http_Client_Exception */
                require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                throw new Sh_Zend_Http_Client_Exception("{$name} is not a valid HTTP header name");
            }
     ...
     * <code>
     * $this->setAuth('shahar', 'secret', Sh_Zend_Http_Client::AUTH_BASIC);
     * </code>
     *
    ...
    public function setAuth($user, $password = '', $type = self::AUTH_BASIC)
    {
        ...
        // Else, set up authentication
            ...
            if (! defined('self::AUTH_' . strtoupper($type))) {
                /** @see Sh_Zend_Http_Client_Exception */
                require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                throw new Sh_Zend_Http_Client_Exception("Invalid or not supported authentication type: '$type'");
            }
     ...
     * and responses.
     *
     * @param Sh_Zend_Http_CookieJar|boolean $cookiejar Existing cookiejar object, true to create a new one, false to disable
     * @return Sh_Zend_Http_Client
     * @throws Sh_Zend_Http_Client_Exception
     */
    public function setCookieJar($cookiejar = true)
    {
        if (! class_exists('Sh_Zend_Http_CookieJar')) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/CookieJar.php';
        }

        if ($cookiejar instanceof Sh_Zend_Http_CookieJar) {
            $this->cookiejar = $cookiejar;
        } elseif ($cookiejar === true) {
            $this->cookiejar = new Sh_Zend_Http_CookieJar();
        } elseif (! $cookiejar) {
            $this->cookiejar = null;
        } else {
            /** @see Sh_Zend_Http_Client_Exception */
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
            throw new Sh_Zend_Http_Client_Exception('Invalid parameter type passed as CookieJar');
        }
     ...
     *
     * @return Sh_Zend_Http_CookieJar|null
     */
     ...
     *
     * @param Sh_Zend_Http_Cookie|string $cookie
    ...
    {
        if (! class_exists('Sh_Zend_Http_Cookie')) {
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Cookie.php';
        }
        ...
        if (isset($this->cookiejar)) {
            if ($cookie instanceof Sh_Zend_Http_Cookie) {
                $this->cookiejar->addCookie($cookie);
            } elseif (is_string($cookie) && $value !== null) {
                $cookie = Sh_Zend_Http_Cookie::fromString("{$cookie}={$value}",
                                                       $this->uri,
            ...
            if (preg_match("/[=,; \t\r\n\013\014]/", $cookie)) {
                /** @see Sh_Zend_Http_Client_Exception */
                require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                throw new Sh_Zend_Http_Client_Exception("Cookie name cannot contain these characters: =,; \t\r\n\013\014 ({$cookie})");
            }
    ...
    public function setFileUpload($filename, $formname, $data = null, $ctype = null)
    {
        if ($data === null) {
            if (($data = @file_get_contents($filename)) === false) {
                /** @see Sh_Zend_Http_Client_Exception */
                require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                throw new Sh_Zend_Http_Client_Exception("Unable to read file '{$filename}' for upload");
            }
     ...
     *
     * @return Sh_Zend_Http_Response or null if none
     */
     ...
     *
     * @param Sh_Zend_Http_Client_Adapter_Interface|string $adapter
     * @return null
     * @throws Sh_Zend_Http_Client_Exception
     */
    public function setAdapter($adapter)
    {
        if (is_string($adapter)) {
            if (!class_exists($adapter)) {
                try {
                    /*require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Loader.php';
                    Sh_Zend_Loader::loadClass($adapter);*/
                  $adapterName = str_replace( 'Sh_Zend_Http_Client_Adapter_', '', $adapter );
                  require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend' . DS . 'Http' . DS . 'Client' . DS . 'Adapter' . DS .ucfirst($adapterName) . '.php'; 
                } catch (Sh_Zend_Exception $e) {
                    /** @see Sh_Zend_Http_Client_Exception */
                    require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                    throw new Sh_Zend_Http_Client_Exception("Unable to load adapter '$adapter': {$e->getMessage()}", 0, $e);
                }
            }
        ...
        if (! $adapter instanceof Sh_Zend_Http_Client_Adapter_Interface) {
            /** @see Sh_Zend_Http_Client_Exception */
            require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
            throw new Sh_Zend_Http_Client_Exception('Passed adapter is not a HTTP connection adapter');
        }
     ...
     * Load the connection adapter
     *
     * @return Sh_Zend_Http_Client_Adapter_Interface $adapter
     */
    ...
    protected function _openTempStream()
    {
        ...
        if (false === ($fp = @fopen($this->_stream_name, "w+b"))) {
                ...
                require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                throw new Sh_Zend_Http_Client_Exception("Could not open temp file {$this->_stream_name}");
        }
    ...
    protected function _prepareHeaders()
    {
        ...
        if (isset($this->cookiejar)) {
            $cookstr = $this->cookiejar->getMatchingCookies($this->uri,
                true, Sh_Zend_Http_CookieJar::COOKIE_STRING_CONCAT);
        ...
        if (count($this->paramsPost) > 0 || count($this->files) > 0) {
            switch($this->enctype) {
                ...
                default:
                    ...
                    /** @see Sh_Zend_Http_Client_Exception */
                    require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                    throw new Sh_Zend_Http_Client_Exception("Cannot handle content type '{$this->enctype}' automatically." .
                        " Please use Sh_Zend_Http_Client::setRawData to send this kind of content.");
                    break;
            }
        }
        ...
        switch ($type) {
            case self::AUTH_BASIC:
                ...
                if (strpos($user, ':') !== false) {
                    /** @see Sh_Zend_Http_Client_Exception */
                    require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                    throw new Sh_Zend_Http_Client_Exception("The user name cannot contain ':' in 'Basic' HTTP authentication");
                }
            ...
            default:
                /** @see Sh_Zend_Http_Client_Exception */
                require_once sh404SEF_ADMIN_ABS_PATH . 'lib/Zend/Http/Client/Exception.php';
                throw new Sh_Zend_Http_Client_Exception("Not a supported HTTP authentication type: '$type'");
        }
