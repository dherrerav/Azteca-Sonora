 ...
 * @version     $Id: com_docman.php 2050 2011-06-30 13:52:38Z silianacom-svn $
 */
...
if( !function_exists( 'dm_sef_get_category_array' ) ){
  function dm_sef_get_category_array( $category_id, $option, $shLangName ){
    ...
    if(empty($tree[$shPageInfo->shMosConfig_locale])){
      ...
      $database->setQuery( $q );
      if (!shTranslateUrl($option, $shLangName))  // V 1.2.4.m
      $DMtree[$shPageInfo->shMosConfig_locale] = $database->loadObjectList( 'id', false);  // V 1.2.4.m if Joomfish, and don't translate
      // use special call of loadObjectList, asking JF not to translate
      else
      $DMtree[$shPageInfo->shMosConfig_locale] = $database->loadObjectList( 'id' );
    }
    $title=array();
    if ($sefConfig->shDMInsertCategories == 1)    // only one category
    $title[] = ($sefConfig->shDMInsertCategoryId ?
    $DMtree[$shPageInfo->shMosConfig_locale][ $category_id ]->id.$sefConfig->replacement : '')
    .$DMtree[$shPageInfo->shMosConfig_locale][ $category_id ]->title;
    else
    do {               // all categories and subcategories. We don't really need id, as path
      $title[] = ($sefConfig->shDMInsertCategoryId ?
      $DMtree[$shPageInfo->shMosConfig_locale][ $category_id ]->id.$sefConfig->replacement : '') // to category
      .$DMtree[$shPageInfo->shMosConfig_locale][ $category_id ]->title;                           // will always be unique
      $category_id = $DMtree[$shPageInfo->shMosConfig_locale][ $category_id ]->parent_id;
    } while( $category_id != 0 );
