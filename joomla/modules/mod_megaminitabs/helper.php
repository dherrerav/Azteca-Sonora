...
/**------------------------------------------------------------------------
* file: helper.php 1.6.0 00001, Mar 2011 12:00:00Z OmegaTheme:Linh $
...
abstract class modMegaMiniTabsHelper
{
	/**
	...
	*/
	public static function getTabsSelection(&$params)
	{
		...
		foreach($list_of_tabs as $key => $value)
		{
			...
			if ($tabItemArr[0] == 'mod')
			{
				$tabArr[] = modMegaMiniTabsHelper::getModules($tabItemArr[1]);
				
			}
			else if ($tabItemArr[0] == 'cat')
			{
				$tabArr[] = modMegaMiniTabsHelper::getArticleList($tabItemArr[1], $params);
			}
	...
	*/
	public static function getArticleList(&$catId, &$params)
	{
		...
		$default_image = $params->get('default_image', 'modules/mod_megaminitabs/images/sampledefault.jpg');
		...
		$cateHtml = array();
		$itemsHtml = '<div class="mmt-items-wrapper">';
		...
		foreach ($items as &$item) {
			...
			$itemsHtml .= '<div class="mmt-item-wrapper">';
			if ($show_thumbnail == 1)
			{
				preg_match_all('/src="([^"]+)"/i', $item->introtext . $item->fulltext, $matches);
				$itemsHtml .= '<div class="mmt-thumb-wrapper">';
				if(empty($matches[1][0])){
					$itemsHtml .= JHTML::_('image', $default_image, $item->title, array('class' => 'mmt-thumbnail', 'width' => $thumb_width, 'height' => $thumb_height));
				}else{
					$itemsHtml .= JHTML::_('image', $matches[1][0], $item->title, array('class' => 'mmt-thumbnail', 'width' => $thumb_width, 'height' => $thumb_height));
				}
				$itemsHtml .='</div>';
			}
			if($show_link == 1)
			{
				$itemsHtml .= '<h3 class="mmt-title">'.
								'<a href="'.$item->link.'" class="mmt-title-link">'.$item->title.'</a>'.
							 '</h3>';
			}
			else
			{
			...
			$itemsHtml .= '<div class="mmt-article-introtext">'.
							JString::substr(strip_tags(preg_replace('/<img([^>]+)>/i',"",JHtml::_('content.prepare', $item->introtext . $item->fulltext))), 0, $number_of_character).'...'.
						 '</div>';
			
			if($show_readmore == 1)
			{
				$itemsHtml .= '<a href="'.$item->link.'" class="mmt-readmore-link">'.JText::_('READMORE').'</a>';
			}
