...
class JDocumentHTML extends JDocument
{
	...
	/**
	* Override the current getBuffer function to fix the issue listed in http://joomlacode.org/gf/project/joomla/tracker/?action=TrackerItemEdit&tracker_item_id=22518
	* The old getBuffer will return the result immediately when found the same $type & $name, don't care about the $attribs
	* This update will make the $attribs is one or parts to build buffer key.
	*/
		...
		if ($result === false) {
			...
			if ($this->_caching == true && $type == 'modules') {
				...
				if (isset($cbuffer[$hash])) {
					...
					$this->setBuffer($renderer->render($name, $attribs, $result), $type, $attname);
					$data = parent::$_buffer[$type][$attname];
